...
def get_delta(model, template, tokenizer, device, args):
    ...
    bs_length = len(tokenizer.encode(template.split(
        ' ')[0].replace('_', ' ').strip())) - 2 + 1
    # replace for roberta tokenizer
    batch = tokenizer(
        [template.replace('_', ' ').strip().replace('   ', ' ')], return_tensors='pt')
    batch['position_ids'] = torch.arange(
        batch['input_ids'].shape[1]).to(device).unsqueeze(0)
    for k in batch:
        batch[k] = batch[k].repeat(256, 1).to(device)
    batch['position_ids'][:,
                          bs_length:] += torch.arange(256).to(device).unsqueeze(-1)
...
def main():
    ...
    parser.add_argument(
        '--mask_embedding_sentence_use_org_pooler', action='store_true')
    parser.add_argument('--mask_embedding_sentence_template',
                        type=str, default=None)
    parser.add_argument('--mask_embedding_sentence_delta', action='store_true')
    parser.add_argument(
        '--mask_embedding_sentence_use_pooler', action='store_true')
    parser.add_argument(
        '--mask_embedding_sentence_autoprompt', action='store_true')
    parser.add_argument('--mask_embedding_sentence_org_mlp',
                        action='store_true')
    ...
    if args.mask_embedding_sentence_org_mlp:
        ...
        mlp = BertForMaskedLM.from_pretrained(
            'bert-base-uncased', config=config).cls.predictions.transform
    ...
    model = AutoModel.from_pretrained(args.model_name_or_path)  # bert
    ...
    if args.p_tuning_prompt:
        if os.path.exists(os.path.join(args.model_name_or_path, "prefix_encoder.ckpt")):
            from easydict import EasyDict
            from transformers import BertConfig
            with open(os.path.join(args.model_name_or_path, "prefix_encoder_model_args.pkl"), "rb") as pkl_file:
                pe_model_args = EasyDict(pickle.load(pkl_file))
            with open(os.path.join(args.model_name_or_path, "prefix_encoder_config.pkl"), "rb") as pkl_file:
                pe_config = pickle.load(pkl_file)
            prefix_encoder = PrefixEncoder(pe_model_args, pe_config)
            prefix_encoder.load_state_dict(torch.load(os.path.join(
                args.model_name_or_path, "prefix_encoder.ckpt")))
            prefix_encoder = prefix_encoder.to(device)
            prefix_tokens = prefix_tokens = torch.arange(
                pe_model_args.pre_seq_len).long().to(device)

            # del pe_config,pe_model_args
        else:
    ...
    if args.mask_embedding_sentence_autoprompt:
        ...
        index_mbv = mask_embedding_template[1:mask_index] + \
            mask_embedding_template[mask_index+1:-1]
    ...
    if args.mask_embedding_sentence_delta:
        delta, template_len = get_delta(
            model, args.mask_embedding_sentence_template, tokenizer, device, args)
    ...
    if args.remove_continue_word:
        ...
        if args.model_name_or_path == 'roberta-base':
            remove_set = {'Ġ.', 'Ġa', 'Ġthe', 'Ġin', 'a', 'Ġ, ', 'Ġis', 'Ġto', 'Ġof', 'Ġand', 'Ġon', 'Ġ\'', 's', '.', 'the', 'Ġman', '-', 'Ġwith',
                          'Ġfor', 'Ġat', 'Ġwoman', 'Ġare', 'Ġ"', 'Ġthat', 'Ġit', 'Ġdog', 'Ġsaid', 'Ġplaying', 'Ġwas', 'Ġas', 'Ġfrom', 'Ġ:', 'Ġyou', 'Ġan', 'i', 'Ġby'}
        else:
            remove_set = {".", "a", "the", "in", ",", "is", "to", "of", "and", "'", "on", "man", "-", "s", "with", "for", "\"", "at", "##s",
                          "woman", "are", "it", "two", "that", "you", "dog", "said", "playing", "i", "an", "as", "was", "from", ":", "by", "white"}
    ...
    def batcher(params, batch, max_length=None):
        ...
        with torch.no_grad():
            if args.embedding_only:
                hidden_states = None
                pooler_output = None
                last_hidden = model.embeddings.word_embeddings(
                    batch['input_ids'])
                position_ids = model.embeddings.position_ids[:,
                                                             0: last_hidden.shape[1]]
                ...
                pooler_output = last_hidden[input_ids ==
            ...
            elif args.p_tuning_prompt:
                batch_size = batch['input_ids'].size(0)
                past_key_values = get_prompt_(
                    pe_model_args, pe_config, prefix_tokens, prefix_encoder, model.device, batch_size)
                prefix_attention_mask = torch.ones(
                    batch_size, pe_model_args.pre_seq_len).to(model.device)
                # print(prefix_attention_mask.shape, batch["attention_mask"].shape)
                prefix_attention_mask = torch.cat(
                    (prefix_attention_mask, batch["attention_mask"]), dim=1)
                outputs = model(input_ids = batch["input_ids"], attention_mask = prefix_attention_mask,
            ...
            # lahelr: new code end here
                ...
                if args.mask_embedding_sentence:
                    last_hidden = outputs.last_hidden_state
                    pooler_output = last_hidden[batch['input_ids']
                                                == tokenizer.mask_token_id]
                    if args.mask_embedding_sentence_org_mlp:
                        pooler_output = mlp(pooler_output)
                    if args.mask_embedding_sentence_delta:
        ...
        if args.mask_embedding_sentence:
            ...
            batch['input_ids'][(batch['input_ids'] == 0) | (batch['input_ids'] == 101) | (
                batch['input_ids'] == 102)] = batch['input_ids'].max()
